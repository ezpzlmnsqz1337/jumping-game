...
export const createLongJumps = (
  scene: BABYLON.Scene,
  level: GameLevel,
  numberOfJumps: number = 19
) => {
  ...
  for (let i = 0; i < numberOfJumps; i++) {
    ...
    const jumpLengthText = BABYLON.MeshBuilder.CreateText(
      '',
      `${jumpLength}`,
    ...
    jumpLengthText.position = new BABYLON.Vector3(
      initialJumpX + 4,
      1.3,
      initialJumpZ + spacing * i
    );
    ...
    material.diffuseColor =
      jumpLength <= 100 ? easyColor : jumpLength <= 110 ? mediumColor : hardColor;
    ...
    walls.push(
      new WallEntity(
        ...
        new BABYLON.Vector3(initialJumpX, 0.3, initialJumpZ + spacing * i)
      ),
      new WallEntity(
        scene,
        level,
        ...
        { width: 8, depth, height },
        new BABYLON.Vector3(
          initialJumpX - nextJumpLengthDelta - 17.5,
          0.3,
          initialJumpZ + spacing * i
        )
      )
  ...
  walls.forEach(
    wall =>
      ((wall.mesh.material as BABYLON.StandardMaterial).diffuseTexture = getLightTexture(
        { uScale: 1, vScale: 1 },
        scene
      ))
  );
